...
$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response["status"] == 1) {
                    $("#" + $module + "-" + $id).html(
                        '<i class="fa-solid fa-toggle-on status"  data-toggle="tooltip" title="Active" style="color:#007bff"' +
                            $module +
                            '_status="Active"></i>'
                    );
                } else if (response["status"] == 0) {
                    $("#" + $module + "-" + $id).html(
                        '<i class="fa-solid fa-toggle-off status"  data-toggle="tooltip" title="Inactive" style="color:grey"' +
                            $module +
                            '_status="Inactive"></i>'
                    );
                }
    ...
    $(document).on("click", ".updateProductAttrStatus", function () {
        ...
        $.ajax({
            ...
            success: function (resp) {
                if (resp["status"] == 1) {
                    $("#productAttr-" + productAttr_id).html(
                        '<i class="fa-solid fa-toggle-on status"  data-toggle="tooltip" title="Active" style="color:#007bff"  productAttr_status="Active"></i>'
                    );
                } else if (resp["status"] == 0) {
                    $("#productAttr-" + productAttr_id).html(
                        '<i class="fa-solid fa-toggle-off status"  data-toggle="tooltip" title="Inactive" style="color:grey"  productAttr_status="Inactive"></i>'
                    );
                }
            },
            error: function () {
    ...
    //update updateCarouselItemStatus

    $(document).on("click", ".updateCarouselitemStatus", function () {
        var status = $(this).find(".status").attr("carouselitem_status");
        var carouselitem_id = $(this).attr("carouselitem_id");

        updateStatus(
            status,
            carouselitem_id,
            "update-carouselitem-status",
            "carouselitem"
        );
    });
    ...
    //show already taken number of display_orders
    // Custom function to validate display order
    // function validateDisplayOrder(
    //     inputFieldId,
    //     takenNumbersContainerId,
    //     takenNumbers
    // ) {
    //     // Display the taken numbers in the UI
    //     $(`#${takenNumbersContainerId}`).text(takenNumbers.join(", "));

    //     // Validate on blur (when the user leaves the input field)
    //     $(`#${inputFieldId}`).on("blur", function () {
    //         var inputVal = $(this).val();
    //         if (inputVal !== "" && takenNumbers.includes(parseInt(inputVal))) {
    //             alert(
    //                 "This display order is already taken. Please choose another one."
    //             );
